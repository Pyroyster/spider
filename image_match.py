...
def distance(img_url, icon_url, display_width):
    ...
    icon_rgb = loadImg(icon_url)
    crop_height = cropHeight(icon_rgb)
    img = cropImage(img_rgb, *crop_height)
    icon = cropImage(icon_rgb, *crop_height)
    src_width = img_rgb.shape[1]
    guess_px = mathc_img(img, icon, value)
